...
- 💡 **突发奇想**：看图的时候想到"这里是不是可以加个 xxx"？直接在当前视图（如用户旅程）上画一个新模块，把现有环节连过去——不是 Scratch，没有预制积木，**模块做什么、连线是什么意思都由你自己写**。点「确认」后 AI 带着现有环节的真实源码位置去实现它（同样走 计划→diff→测试→接受/回滚 闭环）
- 🖱️ 所有地图节点都能拖动摆成你喜欢的样子，箭头自动跟随，位置刷新后保持
